...
class Dashboard:
    ...
    def page_consumer(self):
        st.header("Temperature from Intel Lab Dataset")
        df = pd.read_csv('dataset.csv', delimiter=",")
        st.dataframe(df)
    def page_time_comparing(self):
        col1,col2 = st.columns(2)
        with col1:
            st.header("Current IoT time proccess")
            st.text("Collecting and storing time registration by current IoT")
            df = pd.read_csv('../current_model/commom_IoT_collector.csv', delimiter=",")
            if df is not None:
                self.writeDatasetMetricIot(df)
                
            
            st.header("RFoT time proccess  challenge 5")
            st.text("Collecting and storing time registration by RFoT")
            df = pd.read_csv('../proposed_model/RFoT_collector_challenge_5.csv', delimiter=",")
            if df is not None:
                self.writeDatasetMetricRfot(df)
        ...
        with col2:
            ...
            if df is not None:
                self.writeDatasetMetricRfot(df)
    def page_traning_results(self):
        st.header("Current IoT time proccess")
        st.text("Collecting and storing time registration by RFoT")
        df = pd.read_csv('../consumer/prediction.csv', delimiter=",")
    ...
    def writeDatasetMetricRfot(self,df):
        dfColleting = df[df['Event Description']=='Transaction completed - initing mining']
        dfColleting['Event Duration'] = pd.to_numeric(dfColleting['Event Duration'].str[:4])
        st.metric('average collection time', self.avarageCalc(dfColleting))
        
        dfStroring = df[df['Event Description']=='Block minered']
        dfStroring['Event Duration'] = pd.to_numeric(dfStroring['Event Duration'].str[:4])
        st.metric('average registration time',  self.avarageCalc(dfStroring))
        
        st.dataframe(df.astype(str))

    def writeDatasetMetricIot(self,df):
        dfColleting = df[df['Event Description']=='Transaction completed - initing storing']
        dfColleting['Event Duration'] = pd.to_numeric(dfColleting['Event Duration'].str[:4])
        st.metric('average collection time', self.avarageCalc(dfColleting))
        
        dfStroring = df[df['Event Description']=='transaction registed']
        dfStroring['Event Duration'] = pd.to_numeric(dfStroring['Event Duration'].str[:4])
        st.metric('average registration time', self.avarageCalc(dfStroring))
        st.dataframe(df.astype(str))
    
    def avarageCalc(self,df):
        avarage = df['Event Duration'].sum()/df.shape[0]
        return f'{avarage:.4f}'
